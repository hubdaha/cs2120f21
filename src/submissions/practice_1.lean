-- Jumi Hall, partners: Jake Kauffman, Connor McCaffrey
/-
EQUALITY
-/
...
at first appear.
-/
/- Because we have that z = w, we can show that w = z
through the symmetric property of equality.-/
...
type of X.) 
-/
/- For ∀, you must assume an arbitrary x. In the case of (∀ x, P x) you must 
 then show Px. -/
...
the ∧ elimination rule to p_qr. QED. 
-/
