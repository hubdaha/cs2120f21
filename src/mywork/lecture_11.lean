...
--proving an existentially qualified proposition:
-- there exists some natural number such that n is even (∃ n, ev n)
-- <4 (witness), eq.refl (proof)>
-- predicate ex: pythagorean triple a^2 + b^2 = c^2
...
--∃ (n : ℕ ), red n ∧ blue n → 
--∃ (n : ℕ ), red n.
-- elimination rule: the thing must exist so you can give it a name
